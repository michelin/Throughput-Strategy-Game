package com.michelin.throughputfxproject.tests;
...
class ThroughputTests  {
        ...
        void testDrawBit_WhenRollIsDieFaces_ShouldDrawBitCard() throws IOException {
            // Mock dependencies
            int dieSides = 6;
            when(diceService.getDie(dieSides)).thenReturn(new Die(dieSides));
            when(diceService.rollDie(any(Die.class))).thenReturn(new Die(dieSides)); // Simulate rolling a 6
            ...
            // Call the method
            BitCard result = prompts.drawBit(container, dieSides, gameBoardLog);

            // Verify interactions and assertions
            verify(diceService).rollDie(any(Die.class));
            verify(cardService).pickACardDestructively();
            verify(loader).load();
        ...
        void testDrawBit_WhenRollIsNotDieFaces_ShouldNotDrawBitCard() throws IOException {
            ...
            int dieSides = 6;
            when(diceService.getDie(dieSides)).thenReturn(new Die(dieSides));
            when(diceService.rollDie(any(Die.class))).thenReturn(new Die(1)); // Simulate rolling a 1
            ...
            // Verify interactions and assertions
            verify(diceService).rollDie(any(Die.class));
            verify(cardService, never()).pickACardDestructively();
            assertNull(result);
        }
